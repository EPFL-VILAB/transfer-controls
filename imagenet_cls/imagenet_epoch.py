...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, args.max_images//1000, args.model_name, args.seed)
    ...
    best_name = 'model_best_' + args.model_name + '_' + str(args.max_images//1000) + 'k_'+ str(args.seed) + '.pth.tar'
    if not os.path.isfile(best_name):
        # Model training failed, no best model stored
        best_name = 'checkpoint_' + args.model_name + '_' + str(args.max_images//1000) + 'k_' + str(args.seed) + '.pth.tar'
...
def save_checkpoint(state, is_best, img_num, model_name, seed, filename='checkpoint.pth.tar'):
    filename = 'checkpoint_' + model_name + '_' + str(img_num) + 'k_' + str(seed)  + '.pth.tar'
    torch.save(state, filename)
    if is_best:
        best_name = 'model_best_' + model_name + '_' + str(img_num) + 'k_' + str(seed) + '.pth.tar'
