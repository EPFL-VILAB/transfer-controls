...
from . import RepresentationModule, TaskonomyEncoder
from models import *
from utils import FeatureExtractor, init_conv_dct


class ColorizationTask(RepresentationModule):
    """
    ...
    def __init__(self, pretrained_weights_path, size, pretrained=True):
        super(ColorizationTask, self).__init__()
    ...
    def setup_model(self):
        model = TaskonomyEncoder(eval_only=False, train=True)      # requires_grad = True, we need finetune the whole model

        # modify the conv1 input as 1 channel for gray scale, according to the structure in TaskonomyEncoder
