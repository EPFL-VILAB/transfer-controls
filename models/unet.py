...
class UNet_up_block(nn.Module):
    ...
    def forward(self, x, prev_feature_map=None):
        ...
        if prev_feature_map is not None:
            # Only used for task_encoder
            # bcs it is not a standard resnet50
            _, _, xh, xw = x.size()
            _, _, ph, pw = prev_feature_map.size()
            if xh != ph:
                prev_feature_map = F.interpolate(prev_feature_map, size=xh, mode='nearest')            
...
class UNetDecoder(nn.Module):
    def __init__(self, upsample=6, out_channels=3, use_skip=True):
        super().__init__()
        ...
        self.up_blocks = nn.ModuleList(
            [UNet_up_block(2**(4+i), 2**(5+i), 2**(4+i), use_skip=use_skip) for i in range(0, upsample)]
        )
    ...
    def forward(self, skip_connections):
        '''
        If input is a list, treat it as a list of skip connections and route accordingly.
        If input is a tensor, ignore skip connections and decode only from middle representation.
        '''
        x = skip_connections[-1] if isinstance(skip_connections, Iterable) else skip_connections

        x = self.relu(self.bn1(self.mid_conv1(x)))
        x = self.relu(self.bn2(self.mid_conv2(x)))
        x = self.relu(self.bn3(self.mid_conv3(x)))

        for i in range(0, self.upsample)[::-1]:
            # i = 5, 4, 3, 2, 1, 0
            if isinstance(skip_connections, Iterable):
                x = self.up_blocks[i](x, skip_connections[i])
            else:
                x = self.up_blocks[i](x, None)                
